...
@app.route('/userdata', methods = ['PASS', 'LOGOUT', 'USERS', 'ALLCLASSES', 'GET', 'ADD', 'DROP', 'GETNAME', 'GETPROFILEPICTURE', 'GETFEED','CHANGEGRADE'])
def studPassPull():
    ...
    if (request.method == 'GETNAME'):
        return session['name']
    if (request.method == 'GETPROFILEPICTURE'):
        return "../static/images/userdata/" + str(session['id']) +"/profile/" + session['profilePicture']
    if (request.method == 'GETFEED'):
        # Initializes connection to the junction database
        followingTable_connection = followingTable_engine.connect()
        # Pulls what classes the student is in from junction using their student id, assigns returned list to result
        s = "SELECT * FROM followingTable WHERE user='" + str(session['id']) + "'"
        result = followingTable_connection.execute(s)
        ...
        for row in result:
            ...
            string = string[string.index(',')+2:]
            # Appends important part of data to returnedjson
            followingList += string[:string.index(')')] + ", "

        # Cleans up returnedjson
        # Closes connection to the junction database
        followingTable_connection.close() 
        print(followingList)
        # Connects to classes database
        posts_connection = posts_engine.connect()
        ...
        while (len(followingList) > 1):
            scannedUser = followingList[1:followingList.index(',')]
            followingList = followingList[followingList.index(',')+1:]
            print (followingList)
            # Pulls class data for the key/current class
            s = "SELECT * FROM posts WHERE user='" + scannedUser + "'"
            result = posts_connection.execute(s)
            for row in result:
                # Converts the row of data to a string
                string = str(row)
                # Cuts out unnecessary portions of the data
...
@app.route('/userdata/<calledusername>', methods = ['GET', 'PULLINFO'])
def studPull(calledusername):
    ...
    if (request.method == 'PULLINFO'):
        userdata_connection = userdata_engine.connect()
        # Pulls what classes the student is in from junction using their student id, assigns returned list to result
        s = "SELECT * FROM userdata WHERE id='" + str(calledusername) + "'"
        result = userdata_connection.execute(s)
        returned = str(result.fetchone())
        userdata_connection.close()
        return returned
